...
func (p *Prewarmer) fetchURL(ctx context.Context, url string) *Result {
	...
	resp, err := p.client.Do(req) //nolint:gosec // URL is validated by caller before reaching this point
	if err != nil {
...
func (p *Prewarmer) parseSitemap(ctx context.Context, sitemapURL string) ([]string, error) {
	...
	resp, err := p.client.Do(req) //nolint:gosec // URL comes from validated sitemap entries
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close() //nolint:errcheck // best effort cleanup
